...
func main() {
	...
	movie1 := movie.Movie{redis.GetMoviePriceCode("Star Wars"),"Star Wars I"}
	rental1.Init(movie1, 3)
	aCustomer.AddRental(rental1)

	var rental2 rental.Rental
	movie2 := movie.Movie{redis.GetMoviePriceCode("The Godfather Part II"), "The Godfather Part II"}
	rental2.Init(movie2, 1)
	aCustomer.AddRental(rental2)

	var rental3 rental.Rental
	movie3 := movie.Movie{ redis.GetMoviePriceCode("Casablanca"),"Casablanca I"}
